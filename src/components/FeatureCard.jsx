...
const FeatureCard = ({ icon: Icon, title, description, className = '', variant = 'default', reveal = '', intensity = 'normal' }) => {
  ...
  const revealClass = reveal === 'left' ? 'reveal reveal-left' : reveal === 'right' ? 'reveal reveal-right' : reveal === 'up' ? 'reveal' : 'reveal';
  const intensityClass = intensity === 'strong' ? 'card-3d-strong' : 'card-3d';
